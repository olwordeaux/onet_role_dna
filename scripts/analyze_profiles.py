...
EXPECTED_SECTIONS = ['tasks', 'skills', 'knowledge', 'abilities', 'work_activities', 'work_styles', 'education', 'job_zone']  # noqa: E501
        ...
        if section in details:
            ...
            if isinstance(section_data, list) and len(section_data) > 0:
                ...
                if isinstance(first_item, dict):
                    ...
                    has_rating = any(key in first_item for key in ['importance', 'level', 'percentage', 'value', 'rating'])  # noqa: E501
                    results['ratings'][section] = {
                        'has_rating': has_rating,
                        'keys': list(first_item.keys()) if first_item else []
                    }
                    if not has_rating:
                        results['issues'].append(f"  • {section}: No rating field (importance/level/percentage) found in first item")  # noqa: E501
                else:
